...
def test_validation_single_choice_raise():
    ...
    question = Question("Question 1", QuestionType.SINGLE_CHOICE_QUESTION, answers)
...
def test_validation_multiple_choice_raise():
    answers = [Answer("Answer 1", False), Answer("Answer 2", False)]
    question = Question("Question 1", QuestionType.MULTIPLE_CHOICE_QUESTION, answers)
    with pytest.raises(NotAtLeastOneCorrectAnswersError):
        question.to_dict()
...
def test_validation_text_no_answers():
    answers = []
    question = Question("Question 1", QuestionType.TEXT_QUESTION, answers)
